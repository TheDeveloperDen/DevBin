...
from app.utils.ip import is_ip_in_trusted_hosts, validate_ip_address


class UserMetadataMiddleware(BaseHTTPMiddleware):
    def get_ip_address(self, request: Request) -> ipaddress.IPv4Address | ipaddress.IPv6Address | Literal["unknown"]:
        # Try X-Forwarded-For first (proxy headers)
        forwarded = request.headers.get("X-Forwarded-For")
        ip = "unknown"

        client_host = request.client.host if request.client else None
        is_trusted = client_host and is_ip_in_trusted_hosts(client_host, config.TRUSTED_HOSTS)
