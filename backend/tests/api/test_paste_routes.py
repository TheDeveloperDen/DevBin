...
class TestPasteCreateAPI:
    ...
    async def test_create_paste_with_expiration(self, test_client: AsyncClient, bypass_headers):
    ...
    async def test_create_paste_without_expiration(self, test_client: AsyncClient, bypass_headers):
...
class TestPasteCreateValidationAPI:
    ...
    async def test_create_paste_with_empty_content_fails(self, test_client: AsyncClient, bypass_headers):
    ...
    async def test_create_paste_with_very_large_content(self, test_client: AsyncClient, bypass_headers):
    ...
    async def test_create_paste_with_invalid_language_enum(self, test_client: AsyncClient, bypass_headers):
    ...
    async def test_create_paste_with_unicode_content(self, test_client: AsyncClient, bypass_headers):
    ...
    async def test_create_paste_with_very_long_title(self, test_client: AsyncClient, bypass_headers):
    ...
    async def test_create_paste_with_malformed_json(self, test_client: AsyncClient, bypass_headers):
...
class TestPasteGetAPI:
    ...
    async def test_get_paste_by_id_returns_content(self, test_client: AsyncClient, authenticated_paste, bypass_headers):
    ...
    async def test_get_paste_returns_404_for_nonexistent(self, test_client: AsyncClient, bypass_headers):
    ...
    async def test_get_paste_cache_control_headers(self, test_client: AsyncClient, authenticated_paste, bypass_headers):
    ...
    async def test_get_expired_paste_returns_404(self, test_client: AsyncClient, bypass_headers):
        """GET /pastes/{id} should return 404 for expired paste."""
        from datetime import datetime, timedelta

        # Create a paste that expires immediately
        expired_time = (datetime.now(tz=UTC) - timedelta(hours=1)).isoformat()
        paste_data = {
            "title": "Expired Paste",
...
class TestPasteLegacyAPI:
    ...
    async def test_get_legacy_paste_returns_404_for_nonexistent(self, test_client: AsyncClient, bypass_headers):
...
class TestPasteEditAPI:
    ...
    async def test_edit_paste_with_valid_token_updates_title(self, test_client: AsyncClient, authenticated_paste):
        """PUT /pastes/{id} should update title with valid token."""
        paste_id = authenticated_paste["id"]
        edit_token = authenticated_paste["edit_token"]

        edit_data = {"title": "Updated Title"}
        response = await test_client.put(f"/pastes/{paste_id}", json=edit_data, headers={"Authorization": edit_token})
    ...
    async def test_edit_paste_with_valid_token_updates_content(self, test_client: AsyncClient, authenticated_paste):
    ...
    async def test_edit_paste_with_valid_token_updates_language(self, test_client: AsyncClient, authenticated_paste):
    ...
    async def test_edit_paste_with_valid_token_updates_expiration(self, test_client: AsyncClient, authenticated_paste):
        """PUT /pastes/{id} should update expiration with valid token."""
        paste_id = authenticated_paste["id"]
        edit_token = authenticated_paste["edit_token"]

        new_expiration = (datetime.now(tz=UTC) + timedelta(days=7)).isoformat()
    ...
    async def test_edit_paste_partial_update(self, test_client: AsyncClient, authenticated_paste):
        """PUT /pastes/{id} should support partial updates."""
        paste_id = authenticated_paste["id"]
        edit_token = authenticated_paste["edit_token"]

        # Only update title, leaving other fields unchanged
        edit_data = {"title": "New Title"}
        response = await test_client.put(f"/pastes/{paste_id}", json=edit_data, headers={"Authorization": edit_token})
    ...
    async def test_edit_paste_returns_404_with_invalid_token(self, test_client: AsyncClient, authenticated_paste):
        ...
        response = await test_client.put(
            f"/pastes/{paste_id}", json=edit_data, headers={"Authorization": invalid_token}
        )
    ...
    async def test_edit_paste_returns_404_for_nonexistent_paste(self, test_client: AsyncClient):
        ...
        edit_data = {"title": "Should Fail"}
        response = await test_client.put(
            f"/pastes/{nonexistent_id}", json=edit_data, headers={"Authorization": fake_token}
        )

        assert response.status_code == 404

    async def test_edit_paste_requires_authorization_header(self, test_client: AsyncClient, authenticated_paste):
...
class TestPasteDeleteAPI:
    ...
    async def test_delete_paste_with_valid_token(self, test_client: AsyncClient, authenticated_paste):
        """DELETE /pastes/{id} should succeed with valid token."""
        paste_id = authenticated_paste["id"]
        delete_token = authenticated_paste["delete_token"]

        response = await test_client.delete(f"/pastes/{paste_id}", headers={"Authorization": delete_token})
    ...
    async def test_delete_paste_returns_404_with_invalid_token(self, test_client: AsyncClient, authenticated_paste):
        """DELETE /pastes/{id} should return 404 with invalid token."""
        paste_id = authenticated_paste["id"]
        invalid_token = "invalid_token_12345678901234567890"

        response = await test_client.delete(f"/pastes/{paste_id}", headers={"Authorization": invalid_token})
    ...
    async def test_delete_paste_returns_404_for_nonexistent_paste(self, test_client: AsyncClient):
        """DELETE /pastes/{id} should return 404 for non-existent paste."""
        nonexistent_id = str(uuid.uuid4())
        fake_token = "a" * 32

        response = await test_client.delete(f"/pastes/{nonexistent_id}", headers={"Authorization": fake_token})

        assert response.status_code == 404

    async def test_delete_paste_requires_authorization_header(self, test_client: AsyncClient, authenticated_paste):
...
@pytest.mark.asyncio
async def test_api_compression_is_transparent(test_client: AsyncClient, bypass_headers):
    ...
    edit_response = await test_client.put(f"/pastes/{paste_id}", json=edit_data, headers={"Authorization": edit_token})
